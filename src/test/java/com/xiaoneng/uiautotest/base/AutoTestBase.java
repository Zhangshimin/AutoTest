...
import com.xiaoneng.uiautotest.base.operateFactory.AndroidOperate;
import com.xiaoneng.uiautotest.base.operateFactory.AppOperate;
import com.xiaoneng.uiautotest.base.operateFactory.IosOperate;
import com.xiaoneng.uiautotest.base.operateFactory.WebOperate;
import com.xiaoneng.uiautotest.util.*;
...
public class AutoTestBase {
	...
	/**
	 * Description: Prepare before Suite. 测试套件运行前准备:启动driver server、driver
	 * client，配置相关参数。
	 *
	 * @param filePath
	 *            app的安装包路径
	 ...
	 *            app运行平台的版本
	 * @param deviceName
	 *            app设备名称,ios参数,例如:iPhone 6
	 * @param appPackage
	 *            app包名,android参数,例如:com.xxxx.xxxx
	 * @param appActivity
	 *            android参数,例如:com.xxxx.xxxx
	 * @param port
	 *            app的服务端口
	 * @param udid
	 *            app的设备识别符
	 ...
	 *            web的浏览器类别,例如:chrome
	 * @param remote_url
	 *            web的远程运行的url,例如:http://172.1.2.3:8888/wd/hub
	 */
	@BeforeSuite(alwaysRun = true)
	@Parameters({ "filePath", "appName", "platformName", "platformVersion",
			"deviceName", "appPackage", "appActivity", "port", "udid",
			"timeout", "browser_name", "remote_url" })
	public void beforeSuite(String filePath, String appName,
			String platformName, String platformVersion, String deviceName,
			String appPackage, String appActivity, String port, String udid,
			int timeout, String browser_name, String remote_url) {
		this.port = port;
		...
		if (platformName.toLowerCase().contains("android")
			...
			String filePath2 = new File(filePath).getAbsolutePath();
			File appDir = new File(filePath2);
			File app = new File(appDir, appName);

			StartAppiumServer startAppiumServer = new StartAppiumServer(port,
					udid);
			...
			Log.logInfo("MobileCapabilityType.platformVersion = "
					+ platformVersion);
			...
			if (platformName.toLowerCase().contains("android")) {
				...
				try {
					if (Tools.isMatch(udid, miUdid)) {
						int tapx;
						int tapy;
						Map<String, String> miResolution = StaticConfig
								.getMiResolution();

						tapx = Integer.parseInt(miResolution.get(udid).split(
								"x")[0]);
						tapy = Integer.parseInt(miResolution.get(udid).split(
								"x")[1]);
						HandleSafetyTips handleSafetyTipsOne = new HandleSafetyTips(
								udid, tapx * 7 / 10, tapy * 95 / 100);
						HandleSafetyTips handleSafetyTipsTwo = new HandleSafetyTips(
								udid, tapx * 7 / 10, tapy * 7 / 10);
				...
				try {
					// driver = new EventFiringWebDriver(new IOSDriver(new
					// URL("http://127.0.0.1:"+port+"/wd/hub"),
					// capabilities)).register(executeListener);
					driver = new IOSDriver(new URL("http://127.0.0.1:" + port
							+ "/wd/hub"), capabilities);
				} catch (MalformedURLException e) {
					e.printStackTrace();
				}
			...
			// System.setProperty("log.appium.file", "appium_" + browser_name +
			// ".log");
			...
			if (browser_name.toLowerCase().contains("chrome")) {
				driverServer = Tools.isWindows() ? "chromedriver.exe"
						: "chromedriver";
				processName = Tools.isWindows() ? "chrome.exe"
						: "\"Google Chrome\"";
				...
				System.setProperty("webdriver.safari.driver", driverServerPath
						+ driverServer);
			} else if (browser_name.toLowerCase().contains("opera")) {
				driverServer = Tools.isWindows() ? "operadriver.exe"
						: "operadriver";
				processName = Tools.isWindows() ? "opera.exe" : "Opera";
				System.setProperty("webdriver.opera.driver", driverServerPath
						+ driverServer);
	...
	/**
	 * Description: Back to home page after method. app测试方法执行完成后,回到首页。
	 */
	@AfterMethod(alwaysRun = true)
	public void afterMethod() {
		if (platformName.toLowerCase().contains("android")
				|| platformName.toLowerCase().contains("ios")) {
			String[] flag = { "欢迎来到移动商城" };
			operateBase.backToHomePage(flag);
		}
	}

	/**
	 * Description: Driver quit after suit. 测试套件执行后关闭driver。
	 */
...
//		driver.close();
	...
	/**
	 * Description: Screen Shot. 实现屏幕截屏功能。
	 *
	 * @param fileName
	 *            截屏文件名
	 * @throws IOException
	...
	public static String ScreenShot(String fileName) throws IOException {
		...
		String filePath_testngReports = "output" + File.separator + date
				+ File.separator + time + File.separator + udid
				+ File.separator + "testngReports" + File.separator;
		if (platformName.toLowerCase().contains("android")
				|| platformName.toLowerCase().contains("ios")) {
			filePath_screenShots = "output" + File.separator + date
					+ File.separator + time + File.separator + udid
					+ File.separator + "screenShots" + File.separator;
		} else {
			filePath_screenShots = "output" + File.separator + date
					+ File.separator + time + File.separator + browser_name
					+ File.separator + "screenShots" + File.separator;
		}
		FileUtils.copyFile(
				((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE),
				new File(filePath_screenShots + fileName + ".png"));
